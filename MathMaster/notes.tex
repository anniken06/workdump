...
\subsection{(Definition) Structure}
	...
	- The intepretation $I$ is a function defined by $I: V + OTHER STUFF??? \rightarrow M$ that safisfies the following: \\
...
\subsection{(Definition) Model imported to the language}
	...
	- Imported symbols are denoted by $\overline m$ and $I(\overline m) = I(m)$ \\ TODO $I$ will now accept new shit
	======================================================================
\subsection{(Definition) Interpretation on closed S-terms}
	- TODO: use/meaning/intent - talkables
	- The interpretation $I$ on closed S-terms is defined by the following: \\ TODO: a mapping into M
		- If the term is $a \in Const$, then $I(a)$ from the definition of $I$ \\
		- If the term is $f \in Func$ applied to the closed S-terms $[t_i]_{i=1}^{Arity(f)}$, then $I\left(f([t_i]_{i=1}^{Arity(f)})\right) = I(f)\left([I(t_i)]_{i=1}^{Arity(f)}\right)$ \\
		- The overline counterparts are mapped to the same I mapped
	======================================================================
\subsection{(Definition) Interpretation on closed S-wffs}
	- TODO: use/meaning/inten
	- The interpretation $I$ on closed S-wffs is defined by the following: \\ TODO: a mapping into {t, f}
		- If the wff is $t \equiv s$ on the closed S-terms $t$ and $s$, then $I(t \equiv s) = \top$ iff $I(t) = I(s)$ \\
		- If the wff is $p \in Pred$ applied to the closed S-terms $[t_i]_{i=1}^{Arity(p)}$, then $I\left(p([t_i]_{i=1}^{Arity(p)})\right) = \top$ iff $I(p)\left(I(t_i)]_{i=1}^{Arity(p)}\right) = \top$ \\
		- rule for operator
		- rule for exists
	======================================================================
\subsection{(Definition) S-instance of a structure}
	- The wff $A$ is an S-instance of the the structure $S$ iff for all the free variables $v$ that occur in A, $v \in M$, \\ TODO call imported by something!!!
	- fuck foundations
\begin{comment}
\subsection{() }
	- 
