...
def search_jobs(keywords, page=1, data_collection=[]):
    ...
    if not do_query_listings:
        ...
        with open(os.path.join(Config.data_path, "listings.json"), 'r') as infile: return json.load(infile)
    ...
    with open(os.path.join(Config.data_path, "listings.json"), 'w') as outfile: json.dump(data_collection, outfile, indent=4)
    return data_collection
...
def view_applications(ids):
    ...
    if not do_query_applications:
        ...
        with open(os.path.join(Config.data_path, "applications.json"), 'r') as infile: return json.load(infile)
    if auto_y or input("Run view applications (y)? ") == 'y':
        executor = CommandExecutor()
        raw_executor_results = executor.run_commands_on_workers(
    ...
    with open(os.path.join(Config.data_path, "applications.json"), 'w') as outfile: json.dump(results, outfile, indent=4)
    return results
...
def view_post(ids):
    ...
    if not do_query_posts:
        ...
        with open(os.path.join(Config.data_path, "posts.json"), 'r') as infile: return json.load(infile)
    if auto_y or input("Run view posts (y)? ") == 'y':
        executor = CommandExecutor()
        raw_executor_results = executor.run_commands_on_workers(
            commands=CommandGenerator.generate_commands(view_template, view_replace_dict),
            workers=20)
    ...
    with open(os.path.join(Config.data_path, "posts.json"), 'w') as outfile: json.dump(results, outfile, indent=4)
    return results
...
if __name__ == '__main__':
    ...
    with open(os.path.join(Config.data_path, "melb_posts.json"), 'w') as outfile: json.dump(melb_visa_posts, outfile, indent=4)
